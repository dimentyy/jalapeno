...
(() => {
  ...
  const PION_WS_URL =
    "ws://127.0.0.1:" + (window.PION_PORT || 2903) + "/signaling";
  ...
  function connectPion() {
    let ws = new OrigWebSocket(PION_WS_URL);
    pionWS = ws;
    ws.onopen = function () {
  ...
  function handlePionMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case "remote-track":
        log(
          "remote-track received: kind=" +
            (msg.data && msg.data.kind) +
            " mockPC=" +
            !!mockPC +
            " ontrack=" +
            !!(mockPC && mockPC._ontrack),
        );
        if (mockPC && mockPC._ontrack) {
          let kind = msg.data.kind;
          log("Firing ontrack for remote " + kind);
          let canvas = document.createElement("canvas");
          canvas.width = 2;
          canvas.height = 2;
          let fakeStream = canvas.captureStream(1);
          let fakeTrack =
            kind === "audio"
              ? (function () {
                  let a = new AudioContext();
                  let d = a.createMediaStreamDestination();
                  return d.stream.getAudioTracks()[0];
                })()
              : fakeStream.getVideoTracks()[0];
          log(
            "Fake track created: kind=" +
              fakeTrack.kind +
              " readyState=" +
              fakeTrack.readyState +
              " enabled=" +
              fakeTrack.enabled,
          );
          let evt = new Event("track");
          evt.track = fakeTrack;
          evt.receiver = { track: fakeTrack };
          evt.transceiver = { receiver: { track: fakeTrack } };
          evt.streams = [
            kind === "audio" ? new MediaStream([fakeTrack]) : fakeStream,
          ];
          log(
            "Calling mockPC._ontrack with streams[0].getTracks()=" +
              evt.streams[0].getTracks().length,
          );
          mockPC._ontrack(evt);
      ...
      case "connection-state":
        if (mockPC) {
          ...
          if (
            window.IS_CREATOR &&
            (msg.data === "closed" || msg.data === "failed")
          )
            break;
  ...
  function MockPeerConnection(config) {
    ...
    if (config && config.iceServers) {
      let servers = config.iceServers.map(function (s) {
        return {
  ...
  MockPeerConnection.prototype = {
    ...
    set ontrack(fn) {
      log("MockPC.ontrack SET by caller");
      this._ontrack = fn;
    },
    get ontrack() {
    ...
    setLocalDescription: function (desc) {
      ...
      let oldState = this._signalingState;
      ...
      if (oldState !== this._signalingState) {
        setTimeout(function () {
          if (self._onsignalingstatechange)
            self._onsignalingstatechange(new Event("signalingstatechange"));
          self._listeners.signalingstatechange.forEach(function (fn) {
            fn(new Event("signalingstatechange"));
          });
        }, 0);
      }
    ...
    setRemoteDescription: function (desc) {
      ...
      let self = this;
      return requestPion("set-remote-description", {
        type: desc.type,
        sdp: desc.sdp,
      }).then(function () {
    ...
    removeTrack: function (sender) {
      this._senders = this._senders.filter(function (s) {
        return s !== sender;
      });
    },
    getSenders: function () {
  ...
  let OrigPC = window.RTCPeerConnection;
  ...
  let origStringify = JSON.stringify;
  ...
  JSON.parse = function (str) {
    ...
    if (
      result &&
      (result.type === "notification" || result.type === "response")
    ) {
      try {
  ...
  function handleOutgoing(msg) {
    ...
    if (msg.command === "change-media-settings") {
      log(
        "VK outgoing change-media-settings:",
        JSON.stringify(msg.mediaSettings),
      );
    }
    if (
      msg.command === "join-conversation" ||
      msg.command === "start-conversation"
    ) {
      log(
        "VK outgoing " +
          msg.command +
