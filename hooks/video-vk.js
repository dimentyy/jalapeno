(() => {
    ...
    const PION_WS_URL = "ws://127.0.0.1:" + (window.PION_PORT || 9001) + "/signaling";
    ...
    const reIp4 = /\d+\.\d+\.\d+\.\d+/g;
    const reIp6 = /[0-9a-fA-F]{1,4}(?::[0-9a-fA-F]{1,4}){2,7}|(?:[0-9a-fA-F]{1,4}:){1,6}:[0-9a-fA-F]{1,4}|::(?:[0-9a-fA-F]{1,4}:){0,5}[0-9a-fA-F]{1,4}/g;
    ...
    function requestPion(type, data) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            let msg = JSON.stringify({type: type, data: data, id: id});
            if (pionReady && pionWS && pionWS.readyState === 1) {
                pionWS.send(msg);
            } else {
                pendingMessages.push(msg);
    ...
    MockPeerConnection.prototype = {
        ...
        removeEventListener: function (type, fn) {
            if (this._listeners[type]) this._listeners[type] = this._listeners[type].filter(function (f) {
                return f !== fn;
            });
        },
        dispatchEvent: function (event) {
            let type = event.type;
            if (this._listeners[type]) this._listeners[type].forEach(function (fn) {
                fn(event);
            });
        ...
        addTrack: function (track, stream) {
            let sender = {
                track: track, replaceTrack: function (t) {
                    this.track = t;
                    return Promise.resolve();
        ...
        addTransceiver: function (trackOrKind, init) {
            let kind = typeof trackOrKind === "string" ? trackOrKind : trackOrKind.kind;
            let sender = {
                track: typeof trackOrKind === "string" ? null : trackOrKind, replaceTrack: function (t) {
                    this.track = t;
                    return Promise.resolve();
                }, getParameters: function () {
                    return {encodings: [{}]};
